...
export async function getGoogleClientId(_request, reply) {
    return reply.send({ clientId: process.env.GOOGLE_CLIENT_ID });
}
export async function handleGoogleAuth(request, reply) {
    ...
    try {
        ...
        let user = await db.fetch_one(`SELECT * FROM user WHERE google_id = ? OR email = ? LIMIT 1`, [googleId, email]);
        if (!user) {
            ...
            await db.run(`INSERT INTO user (google_id, email, username, created_at) VALUES (?, ?, ?, ?)`, [googleId, email, username, new Date().toISOString()]);
            user = await db.fetch_one(`SELECT * FROM user WHERE google_id = ? LIMIT 1`, [googleId]);
        }
