...
picture.query_picture_get = "SELECT * FROM profile p WHERE p.user_id = ?";
picture.query_picture_set = "UPDATE profile SET title = ?, picture = ? WHERE p.user_id = ?";
picture.query_picture_create = "INSERT INTO profile (title, picture, user_id) VALUES (?,?,?)";
class match {
    constructor(user_id, fastify) {
        this.user_id = user_id;
        this.fastify = fastify;
...
match.query_get_match = `Select m.id, u1.username as player_1, m.score_player_1, u2.username as player_2, m.score_player_2, w.username as winner from "match" m
...
UserManager.query_info = "SELECT username, email FROM user WHERE id=?";
