...
export default class InviteHandler {
    ...
    private initInviteListeners() {
        this.socket.on("inviteToGame", (inviterUsername: string, inviterId: string) => {
            ...
            console.log(`Invitation received from user ${inviterUsername}`); 
        });

        this.socket.on("getResponse", (invitedId: string, accepted: boolean) => { 
            if (!accepted) this.toggleInviteMsg();
            console.log(`Invitation accepted : ${accepted} by ${invitedId}`); 
        });
    }
    ...
    private async showInvitePopup(inviterUsername: string, inviterId: string) { 
